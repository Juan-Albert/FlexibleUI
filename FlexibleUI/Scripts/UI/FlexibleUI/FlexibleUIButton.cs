...
namespace CamperoEditorTool
{
    ...
    public class FlexibleUIButton : FlexibleUI
    {
        ...
        public void ButtonPressed()
        {
            if (hasSound)
            {
                //AkSoundEngine.PostEvent(soundTapType.ToString(), this.gameObject);
            }

            if (hasPressTween)
            {
                this.transform.DOScale(pressed, duration * 0.5f).SetEase(Ease.OutSine).OnComplete(() =>
                {
                    this.transform.DOScale(Vector3.one, duration * 0.5f).SetEase(Ease.InSine).SetLoops(1, LoopType.Yoyo);
                });
            }
