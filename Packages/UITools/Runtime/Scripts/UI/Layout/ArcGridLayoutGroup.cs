...
namespace UITools
{
	public class ArcGridLayoutGroup : LayoutGroup
	{
	    public float anglePerChild = 5;
		...
		private void SetCellsAlongAxis(int axis)
		{
			...
			for (int i = 0; i < rectChildren.Count; i++)
			{
				...
				{
					float maxAngle = anglePerChild * (rectChildren.Count / 2f);
					angle = Remap(i, 0, rectChildren.Count - 1, -maxAngle, maxAngle);

					float radiusX = cellSize.x + spacing[0];
					float radiusY = (cellSize.y + spacing[1]) * (1 + Mathf.Cos(angle * Mathf.Deg2Rad));

					x = radiusX * (transform.childCount / 2f) * Mathf.Sin(angle * Mathf.Deg2Rad);
					y = radiusY * (transform.childCount / 2f) * (1 - Mathf.Cos(angle * Mathf.Deg2Rad));

					rectChildren[i].localRotation = Quaternion.Euler(new Vector3(0, 0, -angle));
				}

				SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * positionX - x,
					cellSize[0]);
				SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + spacing[1]) * positionY + y,
					cellSize[1]);
			}
		}
